...
### Study Details {.tabset data-height=100}
```{r}
# select <- data %>% 
#   select(Study, Summary = `Summary of Findings`, Domain = `Risk Domain`, Topics = `Topic(s)`) %>% 
#   filter(Study == "A. Patt (2001)")
select <- data.frame(
  Study = "Patt, A. G., & Schrag, D. P. (2003). Using specific language to describe risk and probability. Climatic change, 61(1-2), 17-30. Chicago",
  ...
  DOI = "https://doi.org/10.1023/A:1026314523443"
)
```

`r paste0("**Study**: ", select$Study)` <br>
`r paste0("**Abstract**: ", select$Abstract)` <br>
`r paste0("**DOI**: ", select$DOI)` <br>
...
ggplot(topic_frequency, aes(x = topic, y = prop, fill = prop)) +
