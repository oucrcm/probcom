...
# This chunk was the original version of the above, which I had to revise because the chunk below wouldn't run on my PC for some reason.
# data <- data %>% 
#  rename(
#    cod_1 = Coder...18,
#    cod_2 = Coder...23,
...
### Study Name {.no-title .no-padding data-height=50}

<span style="color: #4682B4;"> <font size="5"> **Study:** </span> `r renderText(select.react() %>% pull(var = Study))` </font> 

### Study Details {.no-title data-height=350}

<span style="color: #4682B4;"> **Abstract:** </span> `r renderText(select.react() %>% pull(var = Abstract))`

<span style="color: #4682B4;"> **Internal Validity:** </span> `r renderText(select.react() %>% pull(var = int))`/3 | <span style="color: #4682B4;"> **External Validity:** </span> `r renderText(select.react() %>% pull(var = ext))`/3 | <span style="color: #4682B4;"> **Domain Validity:** </span> `r renderText(select.react() %>% pull(var = dom))`/3

<span style="color: #4682B4;"> **DOI:** </span> `r renderText(select.react() %>% pull(var = DOI))`
...
DT::datatable(data %>% select(Study, `Summary of Findings`, `Topic(s)`, `Internal Validity` = int, `External Validity` = ext, `Domain Validity` = dom),
              filter = "none",
              selection = "single",
              rownames = FALSE,
              style = "bootstrap",
...
data <- data %>%
  mutate(tableNumber = c(1:nrow(data)))
...
select.react <- reactive({
  ...
  if(is.null(input$table1_row_last_clicked)){
    ...
    data.frame(Study = "Click on one of the studies below to learn more!", Abstract = "None", DOI = "None", int = "0", ext = "0", dom = "0", stringsAsFactors = FALSE)
...
propPlot <- ggplot(topic_frequency, aes(x = topic, y = prop, fill = prop, text = paste0("Topic: ", topic, "<br>", "Proportion of Studies: ", round(prop, 2)*100, "%"))) +
